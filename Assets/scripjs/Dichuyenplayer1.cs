...
public class Dichuyenplayer1 : MonoBehaviour
{   
    public float tocdo;
    public float jump;
    ...
    void Update()
    {
        trai_phai = Input.GetAxis("Horizontal"); //dichuyentraiphai(a d hoac mũi tên trai phai)
        rb.velocity= new Vector2(trai_phai*tocdo,rb.velocity.y);
        flip();
        anim.SetFloat("move", Mathf.Abs(trai_phai));

        if (Input.GetKeyDown(KeyCode.W) && Onjump)
        {
            rb.AddForce(Vector2.up * jump, ForceMode2D.Impulse);
            anim.SetBool("dung", true);
        }
        else  anim.SetBool("dung", false); 
